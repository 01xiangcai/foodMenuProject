...
public class OperationLogAspect {
    ...
    private void setOperatorInfo(com.yao.food_menu.entity.OperationLog log) {
        try {
            // 优先从FamilyContext获取用户ID（拦截器已设置）
            Long currentId = FamilyContext.getUserId();
            
            // 如果FamilyContext中没有，尝试从BaseContext获取（兼容旧代码）
            if (currentId == null) {
            ...
            if (currentId != null) {
                ...
                if (wxUserService != null) {
                    try {
                        ...
                        if (wxUser != null) {
                            log.setOperatorName(wxUser.getNickname() != null ? wxUser.getNickname() : wxUser.getUsername());
