...
public interface ChatConversationMemberMapper extends BaseMapper<ChatConversationMember> {
     ...
     * @return 影响行数
     */
    @Update("UPDATE chat_conversation_member SET unread_count = unread_count + 1, update_time = NOW() " +
            "WHERE conversation_id = #{conversationId} AND wx_user_id != #{excludeUserId} AND deleted = 0")
